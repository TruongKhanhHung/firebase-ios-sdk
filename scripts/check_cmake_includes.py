...
import re
import sys
from typing import (Any, Callable, Collection, FrozenSet, Iterable, Mapping,
...
def run(
  ...
  Arguments:
    ...
    print_func: The function to call to display output to the user; its single
      argument is the string to print/display to the user and its return value
  ...
  print_func(f"Scanned {src_file_count} files; {error_count} error(s) found.")
  return error_count
...
def main(
  ...
  Arguments:
    args: The command-line arguments to use; if None (the default) then
      sys.argv[1:] is used.
    print_func: The function to call to display output to the user; its single
      argument is the string to print/display to the user and its return value
      may be anything, and is ignored; if None (the default) then the built-in
  ...
  args_to_parse = tuple(args) if args is not None else sys.argv[1:]
  parsed_args = argument_parser.parse_args(args_to_parse)
